...
function App() {
  ...
  if (savedSessionRef.current === undefined)
    savedSessionRef.current = readSavedSession();
  ...
  useEffect(() => {
    ...
    const restored = saved.tabs
      .map((entry) => {
        ...
        if (!entry.layout) {
          ...
          return host
            ? { ...createTerminalTab(entry.id, host), ...identity }
            : null;
        }
    ...
    setTabs((prev) => [
      ...prev,
      ...restored.filter((t) => !prev.some((p) => p.id === t.id)),
    ]);
  ...
  const routePrompt = useCallback(
    (prompt, queue) => {
      ...
      const fallback = named
        ? null
        : (() => {
            const tabs = tabsRef.current.filter(
            ...
            const paneId =
              active.focusedPaneId || collectPanes(active.layout)[0]?.id;
      ...
      queue((current) => [
        ...current,
        { ...prompt, tabId: found?.pane.id || prompt.tabId },
      ]);
    },
    [locatePane]
  );
  ...
  useEffect(
    () =>
      window.api.hostKeys.onPrompt((prompt) => {
  ...
  useEffect(
    () =>
      window.api.auth.onPrompt((prompt) => {
  ...
  const hostKeyPromptFor = useCallback(
    (paneId) =>
      hostKeyPrompts.find((prompt) => prompt.tabId === paneId) || null,
    [hostKeyPrompts]
  );
   ...
   */
  const strayHostKeyPrompt =
    hostKeyPrompts.find((prompt) => !paneIds.has(prompt.tabId)) || null;
  ...
  const handleNewTab = useCallback(() => {
    ...
    setTabs((prev) => [
      ...prev,
      { id: tabId, type: "launcher", title: "New Tab" },
    ]);
  ...
  const handleConnect = useCallback((host, intoTabId, view = null) => {
    if (intoTabId) {
      setTabs((prev) =>
        prev.map((tab) =>
          tab.id === intoTabId ? createTerminalTab(intoTabId, host, view) : tab
        )
      );
  ...
  const openAddress = useCallback(async (address) => {
    ...
    toast.error(result?.message || "Could not read that address", {
      style: getToastStyle(),
    });
    return null;
  }, []);
  ...
  const handleConnectResult = useCallback(
    (paneId, result, { reconnect = false } = {}) => {
      ...
      toast.success(
        reconnect
          ? `Reconnected to ${host?.name || "host"}`
          : `Connected to ${host?.name || "host"}`,
        { style: getToastStyle() }
      );
    ...
    const source =
      scope === "window"
        ...
        : tabsRef.current.filter(
            (tab) => tab.type === "terminal" && findPane(tab.layout, paneId)
          );
  ...
  const handleDisconnectTab = useCallback(
    (tabId) => {
  ...
  const handleCloseOtherTabs = useCallback(
    (tabId) => {
      closeTabs(
        tabsRef.current.filter(
          (tab) => tab.type !== "home" && tab.id !== tabId
        ),
        tabId
      );
    },
  ...
  const handleCloseTabsToRight = useCallback(
    (tabId) => {
      ...
      if (at === -1) return;
      closeTabs(
        tabsRef.current.slice(at + 1).filter((tab) => tab.type !== "home"),
        tabId
      );
    },
    [closeTabs]
  ...
  const handleUngroupTab = useCallback((tabId) => {
    setTabs((prev) =>
      prev.map((tab) => (tab.id === tabId ? { ...tab, groupId: null } : tab))
    );
  }, []);
  ...
  const handleNewGroupFromTab = useCallback((tabId) => {
    setTabGroups((groups) => {
      const group = createGroup(
        suggestGroupName(groups),
        nextGroupColor(groups)
      );
  ...
  const handleSplitPane = useCallback(
    (tabId, paneId, direction, source = "duplicate") => {
      const tab = tabsRef.current.find((t) => t.id === tabId);
      if (tab?.type !== "terminal") return;
      ...
      if (paneCount(tab.layout) >= MAX_PANES) {
        toast.error(`A tab holds at most ${MAX_PANES} panes.`, {
          style: getToastStyle(),
        });
        return;
      }
  ...
  const handleClosePane = useCallback(
    (tabId, paneId) => {
      ...
      const position = collectPanes(tab.layout).findIndex(
        (pane) => pane.id === paneId
      );
      const remaining = collectPanes(layout);
      const focused =
        remaining.some((pane) => pane.id === tab.focusedPaneId) &&
      ...
      patchTab(tabId, (current) => ({
        ...
        focusedPaneId: focused,
        zoomedPaneId:
          current.zoomedPaneId === paneId ? null : current.zoomedPaneId,
      }));
    },
  ...
  const handleResizeSplit = useCallback(
    (tabId, splitId, sizes) => {
      patchTab(tabId, (current) => ({
        ...current,
        layout: resizeSplit(current.layout, splitId, sizes),
      }));
    },
    [patchTab]
  );
  ...
  const handleEqualizeSplit = useCallback(
    (tabId, splitId) => {
      patchTab(tabId, (current) => ({
        ...current,
        layout: equalizeSplit(current.layout, splitId),
      }));
    },
    [patchTab]
  );
  ...
  const handlePanePick = useCallback(
    (tabId, paneId, host) => {
      patchTab(tabId, (current) => ({
        ...current,
  ...
  const allTags = useMemo(
    () => tagCounts(hosts).map((entry) => entry.tag),
    [hosts]
  );
  ...
  const handleSaveHost = useCallback(
    async (hostData) => {
      ...
      await saveHost(
        isEditing ? hostData : { ...hostData, folderId: currentFolderId }
      );
      ...
      toast.success(
        isEditing
          ? `Host "${hostData.name}" updated`
          : `Host "${hostData.name}" created`,
        { style: getToastStyle() }
      );
    },
  ...
  useEffect(() => {
    ...
    const off = api.onState((state) => {
      if ((state?.pulled && state.added > 0) || state?.localReset) {
        loadData();
        loadKeys();
      }
    });
      ...
      setTabs((prev) =>
        prev.map((tab) => {
          ...
          const layout = updatePanes(tab.layout, (pane) =>
            pane.host?.id === saved.id
              ? { ...pane, host: { ...pane.host, ...saved } }
              : pane
          );
  ...
  const stripTabs = useMemo(
    () =>
      tabs.map((tab) => {
        ...
        const panes = collectPanes(tab.layout);
        const focused =
          panes.find((pane) => pane.id === tab.focusedPaneId) || panes[0];
        ...
        return {
          ...
          connected:
            sessions.length > 0 && sessions.every((pane) => pane.connected),
  ...
  const currentHosts = hosts.filter(
    (h) => (h.folderId || "") === currentFolderId
  );
  ...
  const connectedHostIds = useMemo(() => {
    ...
    for (const tab of tabs) {
      if (tab.type !== "terminal") continue;
      for (const pane of collectPanes(tab.layout)) {
        if (pane.mode === "terminal" && pane.connected && pane.host?.id)
          ids.add(pane.host.id);
      }
    }
    return ids;
  }, [tabs]);
                ...
                >
                  <SplitLayout
                    ...
                    onResizeSplit={(splitId, sizes) =>
                      handleResizeSplit(tab.id, splitId, sizes)
                    }
                    onEqualizeSplit={(splitId) =>
                      handleEqualizeSplit(tab.id, splitId)
                    }
                    renderPane={(pane, { focused }) =>
                      pane.mode === "picker" ? (
                        <PanePicker
                          ...
                          onPick={(host) =>
                            handlePanePick(tab.id, pane.id, host)
                          }
                          onQuickConnect={async (address) => {
                            const host = await openAddress(address);
                        ...
                        <TerminalView
                          ...
                          onToggleFullscreen={() =>
                            handleToggleFullscreen(tab.id)
                          }
                          onFocus={() => handleFocusPane(tab.id, pane.id)}
                          onSplit={(direction, source) =>
                            handleSplitPane(tab.id, pane.id, direction, source)
                          }
