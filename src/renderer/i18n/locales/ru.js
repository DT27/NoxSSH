...
export default {
    ...
    'hosts.editor.subtitle': 'Куда подключаться, как входить и что делать после входа.',
    ...
    'hosts.editor.telnetWarning': 'Telnet не шифруется. Всё, что по нему передаётся, включая ввод на экране входа, может прочитать любой на пути. Используйте только для устройств, у которых нет ничего лучше.',
    'hosts.editor.hostname': 'Имя хоста / IP',
    'hosts.editor.hostnameDesktopHint': 'Где находится рабочий стол. Используется, если в разделе «Рабочий стол» ниже не указан другой адрес.',
    'hosts.editor.hostnameIpmiHint': 'Где находится контроллер управления. Используется, если в разделе IPMI ниже не указан другой адрес.',
    ...
    'hosts.editor.noKeysInKeychain': 'SSH-ключей нет. Сначала добавьте их на странице «Связка ключей».',
    'hosts.editor.selectKey': 'Выберите ключ…',
    'hosts.editor.password': 'Пароль',
    'hosts.editor.passwordPlaceholderStored': 'Сохранён, оставьте пустым, чтобы не менять',
    ...
    'hosts.editor.privateKeyPlaceholderStored': 'Сохранён, оставьте пустым, чтобы не менять',
    'hosts.editor.privateKeyPlaceholder': '-----BEGIN OPENSSH PRIVATE KEY-----…',
    'hosts.editor.storedPrivateKeyHint': 'Для этого хоста сохранён закрытый ключ.',
    'hosts.editor.keyPassphrase': 'Фраза ключа',
    'hosts.editor.keyPassphrasePlaceholder': 'Оставьте пустым, если у ключа нет фразы',
    ...
    'hosts.editor.displayNameHint': 'Если пусто, хост показывается как {address}.',
    'hosts.editor.displayNamePlaceholder': 'например, Production Server',
    'hosts.editor.tags': 'Метки',
    'hosts.editor.tagsHint': 'Метки не зависят от папок: хост лежит в одной папке и может иметь сколько угодно меток.',
    'hosts.editor.disclosure.connectThrough': 'Подключаться через',
    'hosts.editor.connectDirectly': 'Подключаться напрямую',
    'hosts.editor.jumpHintWith': 'Сначала набор на этот хост; целевой затем достигается по каналу на нём. Сессия внутри остаётся зашифрованной от конца до конца, поэтому релей несёт байты, которые не может прочитать.',
    'hosts.editor.jumpHintWithout': 'Для хоста, до которого с этой машины нет маршрута, укажите бастион, у которого он есть.',
    'hosts.editor.disclosure.proxies': 'Прокси',
    'hosts.editor.proxyHintWith': 'Сокет открывается через прокси, которому поручается достичь адреса выше. Всё, что несёт сессия, идёт внутри него: файлы, проброс портов и напрямую набранный рабочий стол.',
    'hosts.editor.proxyHintWithout': 'Для сети, доступной только через SOCKS или HTTP-прокси. Сохранённые прокси управляются на странице «Прокси».',
    'hosts.editor.noProxiesSaved': 'Прокси не сохранены. Сначала добавьте один на странице «Прокси».',
    'hosts.editor.dialStraightOut': 'Набирать напрямую',
    'hosts.editor.proxyJumpNote': 'Доступ через {jump}; единственное исходящее соединение с этой машины — к {jump}, поэтому его собственный прокси и открывает его. Прокси, выбранный здесь, используется, когда этот хост набирается без релея.',
    'hosts.editor.disclosure.runOnConnect': 'Выполнить при подключении',
    'hosts.editor.initHintSsh': 'Отправляется в оболочку сразу после открытия и снова после переподключения. Одна команда на строку.',
    'hosts.editor.initHintOther': 'Отправляется в момент открытия сессии, ничего не ожидая. Здесь нет определения приглашения, поэтому на устройстве с экраном входа это будет набрано на нём.',
    ...
    'hosts.editor.watchDesc': 'Пока приложение открыто, по таймеру проверять, что здесь ещё кто-то отвечает. Хост, который перестал отвечать, поднимает уведомление один раз, а его карточка помечается, пока он не вернётся.',
    'hosts.editor.checkPort': 'Порт проверки',
    'hosts.editor.checkPortHint': 'Укажите, чтобы следить за чем-то другим на той же машине — веб-сервером или базой, а не за портом входа этого хоста.',
    'hosts.editor.checkPortHintDefault': 'Если пусто, хост проверяется на том порту, по которому к нему подключаются.',
    'hosts.editor.checkPortHintDefaultOn': 'Если пусто, хост проверяется на порту {port}, по которому к нему подключаются.',
    ...
    'hosts.editor.monitoringOffForApp': 'Мониторинг для приложения выключен, поэтому этот хост будет настроен, но пока не проверяться.',
    ...
    'hosts.editor.allowLegacyDesc': 'Включает SHA-1, CBC и 3DES для старых серверов. Ослабляет соединение — оставляйте выключенным, пока рукопожатие не начнёт сбоить.',
    ...
    'keychain.editor.subtitleHello': 'Закрытый ключ находится в TPM этого ПК. Отсюда его не прочитает никто, включая это приложение.',
    'keychain.editor.subtitle': 'Ключи хранятся в связке приложения и не покидают главный процесс.',
    ...
    'keychain.editor.helloBody': 'Закрытый ключ живёт в TPM этого ПК и его нельзя экспортировать, скопировать или сохранить в резервной копии — ни этим приложением, ни вами. Каждое подключение спрашивает Windows Hello.',
    'keychain.editor.helloWarn': 'Он работает только с этой машины. Переустановка в другом месте или сброс Windows Hello уничтожит его навсегда. Держите на серверах другой ключ, чтобы не остаться снаружи.',
    'keychain.editor.publicKey': 'Открытый ключ',
    'keychain.editor.publicKeyOptional': 'Открытый ключ (необязательно)',
    'keychain.editor.publicKeyHelloHint': 'Поместите эту строку в ~/.ssh/authorized_keys на серверах, к которым хотите подключаться.',
    'keychain.editor.publicKeyImportHint': 'Вставьте, чтобы записать отпечаток и алгоритм.',
    ...
    'keychain.editor.typeEd25519': 'Современный, быстрый, безопасный (рекомендуется)',
    ...
    'keychain.editor.keySizeHint': 'Необязательно. Если не выбирать, ssh-keygen возьмёт своё значение по умолчанию.',
    ...
    'keychain.editor.passphraseGenerateHint': 'Необязательно. Созданный ключ шифруется ею.',
    'keychain.editor.passphraseGeneratePlaceholder': 'Оставьте пустым, если фраза не нужна',
    ...
    'keychain.editor.importFromFileHint': 'Выберите ключ вроде id_ed25519. Лежащие рядом .pub или -cert.pub подхватятся сами, и ключ будет прочитан, не проходя через это окно.',
    ...
    'keychain.editor.privateKeyHeld': 'Его хранит приложение и при сохранении зашифрует хранилищем ОС.',
    'keychain.editor.privateKeyStoredPlaceholder': 'Сохранён, оставьте пустым, чтобы не менять',
    ...
    'keychain.editor.certHintEmpty': 'Подписан УЦ и предъявляется вместо голого ключа. Вставьте *-cert.pub.',
    ...
    'keychain.editor.passphraseFileEncrypted': '{file} зашифрован. Без фразы ключ не подключится.',
    'keychain.editor.passphraseStoredHint': 'Для этого ключа сохранена фраза. Оставьте пустым, чтобы не менять.',
    'keychain.editor.passphraseImportHint': 'Только если закрытый ключ выше зашифрован.',
    'keychain.editor.passphraseStoredPlaceholder': 'Сохранена, оставьте пустым, чтобы не менять',
    'keychain.editor.passphraseNonePlaceholder': 'Оставьте пустым, если у ключа нет фразы',
    'keychain.editor.dangerZone': 'Опасная зона',
    'keychain.editor.dangerZoneDesc': 'После удаления ключа вернуть его нельзя. Будьте уверены.',
    ...
    'proxies.editor.subtitle': 'Сервер, через который набирают. Хосты указывают на него, каким бы протоколом они ни говорили после подключения.',
    ...
    'proxies.editor.nameHint': 'Необязательно. Если пусто, прокси показывается как его адрес.',
    ...
    'proxies.editor.identHint': 'Отправляется открытым текстом как идентификатор пользователя SOCKS4. Большинство прокси его игнорируют; оставляйте пустым, пока ваш его не проверяет.',
    ...
    'proxies.editor.viaHintWith': 'Сначала набор на него; этот прокси затем достигается через него. Каждый знает только о следующем, поэтому дальний конец видит последний прокси в маршруте.',
    'proxies.editor.viaHintWithout': 'Для прокси, недоступного с этой машины напрямую, укажите прокси, который его видит.',
    ...
    'proxies.editor.remoteDnsSocks4': 'Отправляет имя, а не адрес — это добавил SOCKS4a. Выключайте только для слишком старого прокси, который имя не принимает.',
    'proxies.editor.remoteDnsDesc': 'Держит DNS вне этой машины, поэтому здесь не узнают, к какому хосту вы идёте, а разделённые сети разрешаются на той стороне, где это возможно. Выключайте для прокси, который отказывается от имён.',
    'proxies.editor.timeout': 'Сдаться через',
    'proxies.editor.timeoutHint': 'Сколько прокси даётся, чтобы принять соединение и ответить. Сама сессия этим не обрывается.',
    ...
    'snippets.editor.subtitle': 'Команда, которую храните и вставляете в сеанс из палитры.',
    'snippets.editor.subtitlePackage': 'Серия команд, отправляемых в сеанс по порядку.',
    ...
    'snippets.editor.kindCommandHint': 'Один фрагмент текста, который кладётся в приглашение.',
    'snippets.editor.kindPackageHint': 'Шаги выполняются в заданном порядке. Шаг можно написать здесь или взять из библиотеки.',
    'snippets.editor.name': 'Имя',
    'snippets.editor.namePlaceholder': 'например, Хвост ошибок nginx',
    'snippets.editor.namePlaceholderPackage': 'например, Развернуть и перезапустить',
    'snippets.editor.command': 'Команда',
    'snippets.editor.commandHint': 'То, о чём нужно спросить, оберните двойными фигурными скобками, например {{service}}.',
    ...
    'snippets.editor.descriptionPlaceholder': 'Что делает или когда к нему тянуться',
    ...
    'snippets.editor.noHostWarning': 'Без выбранного хоста этот сниппет нигде не появится.',
    'snippets.editor.runImmediately': 'Запускать сразу после вставки',
    'snippets.editor.runImmediatelyCommand': 'Нажимает Enter за вас. Выключите, чтобы положить команду в приглашение и прочитать перед запуском.',
    'snippets.editor.runImmediatelyPackage': 'Нажимает Enter за вас и запускает всю серию. Выключите, чтобы сначала положить шаги в приглашение.',
    'folders.editor.titleNew': 'Новая папка',
    'folders.editor.titleEdit': 'Изменить папку',
    'folders.editor.subtitle': 'Папки группируют хосты. Удаление папки не удаляет то, что внутри.',
    ...
    'settings.assistant.provider.relay': 'OpenAI-совместимый релей. Claude Code, Codex и OpenCode устанавливать не нужно.',
    ...
    'settings.assistant.relayNote': 'Укажите OpenAI-совместимый адрес, например https://example.com/v1. Локальный CLI не нужен.',
    ...
    'settings.assistant.relayModelsEmpty': 'Релей не вернул список моделей. Можно ввести имя вручную.',
    'settings.assistant.relayModelsFailed': 'Не удалось загрузить модели. Проверьте адрес и ключ.',
    'settings.assistant.accountRelay': 'Модель вызывается через релей. Укажите адрес и API-ключ. Локальный агент не нужен.',
    ...
    'settings.account.webdavUrlHint': 'Достаточно адреса сервера. Путь noxssh/snapshot.json добавляется автоматически.',
    'settings.account.username': 'Имя пользователя',
    'settings.account.webdavPassword': 'Пароль WebDAV',
    'settings.account.webdavPasswordHint': 'Пароль HTTP Basic для вашего WebDAV-сервера (хранится безопасно).',
    'settings.account.syncPassphrase': 'Фраза синхронизации',
    'settings.account.syncPassphraseHint': 'Шифрует снимок перед загрузкой. Она понадобится на других устройствах для восстановления.',
    ...
    'settings.account.enableSyncDesc': 'Автоматически отправлять и получать зашифрованные хосты, ключи и настройки.',
    ...
    'settings.account.webdavPrivacyNote': 'Данные шифруются на этом устройстве фразой синхронизации до отправки. Пароль WebDAV нужен только для входа.',
    'settings.account.backupSection': 'Исторические копии',
    'settings.account.backupEnabled': 'Включить исторические копии',
    'settings.account.backupEnabledDesc': 'По расписанию создавать на сервере отдельные зашифрованные файлы с меткой времени, отдельно от живого снимка.',
