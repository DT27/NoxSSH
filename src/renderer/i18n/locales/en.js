...
export default {
    ...
    'hosts.editor.subtitle': 'Where to connect, how to authenticate, and what to do once you are in.',
    ...
    'hosts.editor.telnetWarning': 'Telnet is unencrypted. Everything sent over it, including whatever you type at a login prompt, is readable by anything on the path. Use it for devices that offer nothing better.',
    'hosts.editor.hostname': 'Hostname / IP',
    'hosts.editor.hostnameDesktopHint': 'Where the desktop is. Used unless the Desktop section below names a different address.',
    'hosts.editor.hostnameIpmiHint': 'Where the service processor is. Used unless the IPMI section below names a different address.',
    ...
    'hosts.editor.noKeysInKeychain': 'No SSH keys found. Add keys in the Keychain page first.',
    ...
    'hosts.editor.keyPassphrasePlaceholder': 'Leave empty if the key has no passphrase',
    ...
    'hosts.editor.displayNameHint': 'Left blank, this host is listed as {address}.',
    'hosts.editor.displayNamePlaceholder': 'e.g. Production Server',
    'hosts.editor.tags': 'Tags',
    'hosts.editor.tagsHint': 'Cuts across folders: a host sits in one folder and carries as many tags as you like.',
    'hosts.editor.disclosure.connectThrough': 'Connect through',
    'hosts.editor.connectDirectly': 'Connect directly',
    'hosts.editor.jumpHintWith': 'Dialled first; this host is then reached over a channel on it. The session inside stays encrypted end to end, so the relay carries bytes it cannot read.',
    'hosts.editor.jumpHintWithout': 'For a host with no route from this machine, name the bastion that does have one.',
    'hosts.editor.disclosure.proxies': 'Proxies',
    'hosts.editor.proxyHintWith': 'The socket is opened through the proxy, which is asked to reach the address above. Everything the session carries travels inside it: files, port forwards and a directly dialled desktop alike.',
    'hosts.editor.proxyHintWithout': 'For a network only reachable through a SOCKS or HTTP proxy. Saved proxies are managed on the Proxies page.',
    'hosts.editor.noProxiesSaved': 'No proxies saved. Add one on the Proxies page first.',
    'hosts.editor.dialStraightOut': 'Dial straight out',
    'hosts.editor.proxyJumpNote': 'Reached through {jump}, the only connection out of this machine is the one to {jump}, so its own proxy setting is what opens it. The proxy chosen here is used when this host is dialled without the relay.',
    'hosts.editor.disclosure.runOnConnect': 'Run on connect',
    'hosts.editor.initHintSsh': 'Sent to the shell as soon as it opens, and again after a reconnect. One command per line.',
    'hosts.editor.initHintOther': 'Sent the moment the session opens, with nothing waited for. There is no prompt detection here, so on a device that asks for a login this is typed at the login prompt.',
    ...
    'hosts.editor.watchDesc': 'While the app is open, check on a timer that something is still answering here. A host that stops raises a notification once, and its card is marked until it comes back.',
    'hosts.editor.checkPort': 'Check port',
    'hosts.editor.checkPortHint': 'Set it to watch something else on the same machine, a web server or a database, rather than the login this host is reached by.',
    'hosts.editor.checkPortHintDefault': 'Left blank, this host is checked on the port it connects on.',
    'hosts.editor.checkPortHintDefaultOn': 'Left blank, this host is checked on port {port}, the one it connects on.',
    ...
    'hosts.editor.monitoringOffForApp': 'Monitoring is off for the app, so this host will be set up and not yet checked.',
    ...
    'hosts.editor.allowLegacyDesc': 'Enables SHA-1, CBC and 3DES for old servers. Weakens the connection, so leave off unless the handshake fails.',
    ...
    'keychain.editor.subtitleHello': 'The private key is in this PC’s TPM. Nothing here can read it, including this app.',
    'keychain.editor.subtitle': 'Keys live in the app keychain and never leave the main process.',
    ...
    'keychain.editor.helloBody': 'The private key lives in this PC’s TPM and cannot be exported, copied or backed up, not by this app and not by you. Every connection asks for Windows Hello.',
    'keychain.editor.helloWarn': 'It only works from this machine. Reinstalling elsewhere, or resetting Windows Hello, loses it for good. Keep another key on your servers so that does not lock you out.',
    'keychain.editor.publicKey': 'Public key',
    'keychain.editor.publicKeyOptional': 'Public key (optional)',
    'keychain.editor.publicKeyHelloHint': 'Put this line in ~/.ssh/authorized_keys on the servers you want to reach.',
    'keychain.editor.publicKeyImportHint': 'Paste it to record the fingerprint and the algorithm.',
    ...
    'keychain.editor.keySizeHint': 'Optional. Left alone, ssh-keygen picks its own default.',
    ...
    'keychain.editor.passphraseGenerateHint': 'Optional. The generated key is encrypted with this.',
    'keychain.editor.passphraseGeneratePlaceholder': 'Leave empty for no passphrase',
    ...
    'keychain.editor.importFromFileHint': 'Pick a key such as id_ed25519. A .pub or -cert.pub sitting beside it comes too, and the key is read without passing through this window.',
    ...
    'keychain.editor.privateKeyHeld': 'It is held by the app and will be encrypted with the OS keystore when you save.',
    'keychain.editor.privateKeyStoredPlaceholder': 'Stored, leave blank to keep the existing key',
    ...
    'keychain.editor.certHintEmpty': 'Signed by a CA and presented instead of the bare key. Paste a *-cert.pub.',
    ...
    'keychain.editor.passphraseFileEncrypted': '{file} is encrypted. Without its passphrase the key cannot connect.',
    'keychain.editor.passphraseStoredHint': 'A passphrase is stored for this key. Leave blank to keep it.',
    'keychain.editor.passphraseImportHint': 'Only if the private key above is encrypted.',
    'keychain.editor.passphraseStoredPlaceholder': 'Stored, leave blank to keep',
    'keychain.editor.passphraseNonePlaceholder': 'Leave empty if the key has none',
    'keychain.editor.dangerZone': 'Danger zone',
    'keychain.editor.dangerZoneDesc': 'Once you delete this key, there is no going back. Please be certain.',
    ...
    'proxies.editor.subtitle': 'A server to dial through. Hosts point at it, whatever they speak once they are connected.',
    ...
    'proxies.editor.nameHint': 'Optional. Left blank, this proxy is listed as its address.',
    ...
    'proxies.editor.identHint': 'Sent in the clear as the SOCKS4 user id. Most proxies ignore it; leave it blank unless yours is checking it.',
    ...
    'proxies.editor.viaHintWith': 'Dialled first; this proxy is then reached through it. Each one only ever knows about the next, so the far end sees the last proxy in the route.',
    'proxies.editor.viaHintWithout': 'For a proxy that is not reachable from this machine directly, name the proxy that can see it.',
    ...
    'proxies.editor.remoteDnsSocks4': 'Sends the name rather than an address, which is what SOCKS4a added. Turn it off only for a proxy too old to accept one.',
    'proxies.editor.remoteDnsDesc': 'Keeps DNS off this machine, so nothing here learns which host you are reaching, and split networks resolve on the side that can. Turn it off for a proxy that refuses hostnames.',
    'proxies.editor.timeout': 'Give up after',
    'proxies.editor.timeoutHint': 'How long the proxy has to accept the connection and answer. The session itself is never cut off by this.',
    ...
    'snippets.editor.subtitle': 'A command you keep around, sent into a session from the palette.',
    'snippets.editor.subtitlePackage': 'A series of commands, sent into a session in order.',
    ...
    'snippets.editor.kindCommandHint': 'One piece of text, dropped at the prompt.',
    'snippets.editor.kindPackageHint': 'Steps run in the order you set. A step can be written here or taken from the library.',
    ...
    'snippets.editor.commandHint': 'Wrap anything you want to be asked for in double braces, e.g. {{service}}.',
    ...
    'snippets.editor.descriptionPlaceholder': 'What it does, or when to reach for it',
    ...
    'snippets.editor.noHostWarning': 'With no host selected this snippet will not appear anywhere.',
    'snippets.editor.runImmediately': 'Run as soon as it is inserted',
    'snippets.editor.runImmediatelyCommand': 'Presses Enter for you. Leave off to drop the command at the prompt so it can be read before it runs.',
    'snippets.editor.runImmediatelyPackage': 'Presses Enter for you, which starts the whole series. Leave off to drop the steps at the prompt so they can be read before anything runs.',

    'folders.editor.titleNew': 'New folder',
    'folders.editor.titleEdit': 'Edit folder',
    'folders.editor.subtitle': 'Folders group hosts. Deleting one keeps whatever was inside it.',
    ...
    'settings.assistant.provider.relay': 'Uses an OpenAI-compatible relay. No local Claude Code, Codex or OpenCode install required.',
    ...
    'settings.assistant.relayNote': 'Enter an OpenAI-compatible endpoint, e.g. https://example.com/v1. No local CLI is required.',
    ...
    'settings.assistant.relayModelsEmpty': 'The relay returned no models. You can type a model name instead.',
    'settings.assistant.relayModelsFailed': 'Could not fetch models. Check the URL and API key.',
    'settings.assistant.accountRelay': 'Talks to the model through a relay. Enter a base URL and API key. No local agent is required.',
    ...
    'settings.account.webdavUrlHint': 'Enter only the server base address. "noxssh/snapshot.json" is appended automatically.',
    'settings.account.username': 'Username',
    'settings.account.webdavPassword': 'WebDAV Password',
    'settings.account.webdavPasswordHint': 'HTTP Basic Auth password for your WebDAV server (stored securely).',
    'settings.account.syncPassphrase': 'Sync Passphrase',
    'settings.account.syncPassphraseHint': 'Used to encrypt the snapshot before uploading. You will need this passphrase on any other device to restore.',
    ...
    'settings.account.enableSyncDesc': 'Automatically push and pull your encrypted hosts, keys and settings.',
    ...
    'settings.account.webdavPrivacyNote': 'Your data is encrypted on this device with the sync passphrase before it leaves. The WebDAV password is only used for authentication.',
    'settings.account.backupSection': 'Historical backups',
    'settings.account.backupEnabled': 'Enable historical backups',
    'settings.account.backupEnabledDesc': 'Create timestamped encrypted backup files on the server on a schedule, separate from the live snapshot.',
