...
function describeCertificate(key) {
  ...
  if (details.kind === "host") {
    throw new Error(
      "That is a host certificate; a key needs a user certificate to log in with"
    );
  }
...
function register(getWindow) {
  ...
  powerMonitor.on("resume", () =>
    notify("system-resume", { reason: "resume" })
  );
  powerMonitor.on("unlock-screen", () =>
    notify("system-resume", { reason: "unlock" })
  );
   ...
   */
  const requestKeyboardInteractive = (details) =>
    new Promise((resolve) => {
      const window = getWindow();
      if (!window || window.isDestroyed()) {
        resolve(null);
  ...
  handle("auth-prompt-response", (event, { requestId, answers }) => {
    ...
    resolve(
      Array.isArray(answers)
        ? answers.map((answer) => String(answer ?? ""))
        : null
    );
    return true;
  });
  ...
  handle("host-quick-connect", (event, address) => {
    ...
    return host
      ...
      : {
          success: false,
          message: "That is not an address this can dial",
          host: null,
        };
  });
  ...
  handle("save-key", (event, key) => {
    ...
    return store.saveKey({
      ...rest,
      ...keygen.identify(rest),
      ...describeCertificate(rest),
    });
  });
  ...
  handle("delete-key", async (event, keyId) => {
    const credential = store
      .getKeys()
      .find((key) => key.id === keyId)?.helloCredential;
  ...
  handle("generate-key", async (event, options) => {
    const { privateKey, publicKey, fingerprint } = await keygen.generate(
      options
    );
  ...
  handle("import-key-file", async (event, { replaces } = {}) => {
    ...
    try {
      ...
      if (size > MAX_KEY_BYTES) {
        return {
          success: false,
          message:
            `That file is ${Math.round(size / 1024)} KB. No private key is ` +
    ...
    if (inspected.state === "ppk") {
      return {
        success: false,
        message:
          "That is a PuTTY .ppk file, which this cannot read as it stands. " +
    ...
    if (inspected.state !== "ready" && inspected.state !== "encrypted") {
      return {
        success: false,
        message: inspected.reason || "That file is not a private key",
      };
    }
  ...
  handle("delete-snippet", (event, snippetId) =>
  ...
  handle("app-lock-set", (event, password) =>
    logLock(
      "lock.enable",
      vault.set(password),
      "An opening password is now required"
    )
  );
  ...
  handle("app-lock-disable", (event, password) =>
    logLock(
      ...
      "Stored secrets are no longer behind a password"
    )
  );
  ...
  handle("known-hosts-forget-key", (event, { id, fingerprint }) =>
    logForget(
      ...
      knownHosts.forgetFingerprint(id, fingerprint)
    )
  );
  ...
  const describeForward = (tabId, tunnelId) => {
    ...
    return found.name
      ? `${found.name} (${describeTunnel(found)})`
      : describeTunnel(found);
  };
  ...
  handle("tunnels-sync", (event, { tabId, hostId }) =>
    tunnels.sync(tabId, hostId)
  );
  ...
  handle("vnc-name", (event, { paneId, name }) =>
    vnc.setDesktopName(paneId, name)
  );
   ...
   */
  handle("bmc-attach", (event, { paneId, webContentsId }) =>
    bmc.attach(paneId, webContentsId)
  );
    ...
    activity.record({
      ...
      detail:
        [
          ...
          result?.knownHosts?.imported
            ? `${result.knownHosts.imported} known host(s)`
            : "",
          ...
          result?.hosts?.relayed
            ? `${result.hosts.relayed} linked to a jump host`
            : "",
        ]
          .filter(Boolean)
  ...
  handle("backup-export", async (event, { passphrase } = {}) => {
    ...
    try {
      ...
      return {
        success: false,
        message: `Could not write the backup: ${error.message}`,
      };
    }
  ...
  handle("backup-restore", (event, { token, overwrite } = {}) => {
    ...
    if (!payload) {
      return {
        success: false,
        message: "That backup is no longer open. Choose the file again.",
      };
    }
  ...
  handle(
    "webdav-sync-test",
    async (event, { url, username, password } = {}) => {
      return webdavSync.test({ url, username, password });
    }
  );

  handle("webdav-sync-set-enabled", (event, enabled) =>
    webdavSync.setEnabled(enabled)
  );
  ...
  handle("webdav-sync-delete-backup", async (event, { name } = {}) => ({
    result: await webdavSync.deleteBackup(name),
    status: webdavSync.status(),
  }));

  handle("webdav-sync-reset-local", async () => ({
    result: await webdavSync.resetLocalData(),
    status: webdavSync.status(),
  }));
  ...
  handle("sftp-list", (event, { tabId, remotePath }) =>
    sftp.list(tabId, remotePath)
  );
  handle("sftp-home", (event, tabId) => sftp.home(tabId));
  handle("sftp-realpath", (event, { tabId, remotePath }) =>
    sftp.realpath(tabId, remotePath)
  );
  ...
  handle("sftp-mkdir", (event, { tabId, remotePath }) =>
    logFile("file.mkdir", tabId, remotePath, "Folder created", () =>
      sftp.mkdir(tabId, remotePath)
    )
  );

  handle("sftp-create-file", (event, { tabId, remotePath }) =>
    logFile("file.create", tabId, remotePath, "File created", () =>
      sftp.createFile(tabId, remotePath)
    )
  );
  ...
  handle("sftp-chmod", (event, { tabId, remotePath, mode, recursive }) =>
    logFile(
      ...
      tabId,
      remotePath,
      `${((mode || 0) & 0o7777).toString(8).padStart(4, "0")}${
  ...
  handle("activity-export", async () => {
    ...
    try {
      ...
      return {
        success: false,
        message: `Could not write the log: ${error.message}`,
      };
    }
  ...
  handle("session-log-configure", (event, patch) =>
    sessionLog.setConfig(patch || {})
  );
  handle("session-log-status", (event, tabId) => sessionLog.status(tabId));
  handle("session-log-list", (event, options) =>
    sessionLog.list(options || {})
  );
  ...
  handle("session-log-start", (event, tabId) => {
    ...
    if (!session)
      return { success: false, message: "That session is not connected" };
    ...
    return filePath
      ...
      : {
          success: false,
          message: "Could not open a log file. Check the folder in Settings.",
        };
  });
  ...
  handle("session-log-choose-directory", async () => {
    ...
    return {
      success: true,
      config: sessionLog.setConfig({ directory: filePaths[0] }),
    };
  });
  ...
  handle("session-log-reset-directory", () => ({
    success: true,
    config: sessionLog.setConfig({ directory: "" }),
  }));
  ...
  handle("session-log-reveal", async (event, filePath) => {
    ...
    if (resolved !== root && !resolved.startsWith(root + path.sep)) {
      return {
        success: false,
        message: "That file is not in the session log folder",
      };
    }

    if (!fs.existsSync(resolved))
      return { success: false, message: "That log is no longer there" };
  ...
  handle("sftp-transfer-cancel-all", (event, tabId) =>
    transfers.cancelAll(tabId)
  );
  handle("sftp-transfer-retry", (event, id) => transfers.retry(id));
  handle("sftp-transfer-clear", (event, tabId) =>
    transfers.clearFinished(tabId)
  );
  ...
  handle("choose-logo-image", async () => {
    const { canceled, filePaths } = await dialog.showOpenDialog(getWindow(), {
      ...
      filters: [
        {
          name: "Images",
          extensions: Object.keys(LOGO_TYPES).map((ext) => ext.slice(1)),
        },
      ],
    });
    ...
    if (!type)
      return {
        success: false,
        message: "That file is not an image the title bar can draw",
      };
    ...
    try {
      ...
      if (size > MAX_LOGO_BYTES) {
        return {
          success: false,
          message:
            `That image is ${Math.round(size / 1024)} KB. The limit is ` +
            `${
              MAX_LOGO_BYTES / 1024
            } KB, because the mark is drawn 24 pixels square ` +
  ...
  handle(
    "show-open-dialog",
    (event, { properties, filters, defaultPath, title }) =>
  ...
  handle("ai-conversation-start", (event, payload) =>
    assistant.create(payload || {})
  );
  handle("ai-conversation-list", () => assistant.list());
  handle("ai-conversation-history", (event, conversationId) =>
    assistant.history(conversationId)
  );
  handle("ai-conversation-park", (event, conversationId) =>
    assistant.park(conversationId)
  );
  handle("ai-conversation-close", (event, conversationId) =>
    assistant.close(conversationId)
  );
  handle("ai-scope", (event, payload) =>
    assistant.setScope(payload?.conversationId, payload || {})
  );
  handle("ai-send", (event, payload) =>
    assistant.send(payload?.conversationId, payload?.text)
  );
  handle("ai-interrupt", (event, conversationId) =>
    assistant.interrupt(conversationId)
  );
  ...
  // asked to.
  handle("ai-approval-response", (event, payload) =>
    assistant.respondToApproval(payload || {})
  );
  handle("ai-action-response", (event, payload) =>
    assistant.respondToAction(payload || {})
  );
  ...
  handle("startup-set-enabled", (event, enabled) =>
