...
contextBridge.exposeInMainWorld("api", {
  hosts: {
    ...
    quickConnect: (address) =>
      ipcRenderer.invoke("host-quick-connect", String(address || "")),
  },
  ...
  sessionLog: {
    ...
    configure: (patch) =>
      ipcRenderer.invoke("session-log-configure", patch || {}),
  ...
  appLock: {
    ...
    change: (current, next) =>
      ipcRenderer.invoke("app-lock-change", { current, next }),
  ...
  sftp: {
    ...
    list: (tabId, remotePath) =>
      ipcRenderer.invoke("sftp-list", { tabId, remotePath }),
    home: (tabId) => ipcRenderer.invoke("sftp-home", tabId),
    realpath: (tabId, remotePath) =>
      ipcRenderer.invoke("sftp-realpath", { tabId, remotePath }),
    stat: (tabId, remotePath, follow = true) =>
      ipcRenderer.invoke("sftp-stat", { tabId, remotePath, follow }),
    diskUsage: (tabId, remotePath) =>
      ipcRenderer.invoke("sftp-disk-usage", { tabId, remotePath }),

    mkdir: (tabId, remotePath) =>
      ipcRenderer.invoke("sftp-mkdir", { tabId, remotePath }),
    createFile: (tabId, remotePath) =>
      ipcRenderer.invoke("sftp-create-file", { tabId, remotePath }),
    remove: (tabId, remotePaths) =>
      ipcRenderer.invoke("sftp-delete", { tabId, remotePaths }),
  ...
  transfers: {
    enqueue: (tabId, options) =>
      ipcRenderer.invoke("sftp-transfer-enqueue", { tabId, ...options }),
    ...
    onConflictResolved: (callback) =>
      subscribe("sftp-conflict-resolved", callback),
  },

  tunnels: {
    list: (tabId) => ipcRenderer.invoke("tunnels-list", tabId),
    sync: (tabId, hostId) =>
      ipcRenderer.invoke("tunnels-sync", { tabId, hostId }),
    start: (tabId, tunnelId) =>
      ipcRenderer.invoke("tunnels-start", { tabId, tunnelId }),
    stop: (tabId, tunnelId) =>
      ipcRenderer.invoke("tunnels-stop", { tabId, tunnelId }),
  ...
  vnc: {
    ...
    open: (paneId, hostId) =>
      ipcRenderer.invoke("vnc-open", { paneId, hostId }),
    close: (paneId, hostId) =>
      ipcRenderer.invoke("vnc-close", { paneId, hostId }),
    // Reported back so the header and the activity log can name the desktop
    // the way the server does.
    reportName: (paneId, name) =>
      ipcRenderer.invoke("vnc-name", { paneId, name }),
  ...
  rdp: {
    ...
    open: (paneId, hostId) =>
      ipcRenderer.invoke("rdp-open", { paneId, hostId }),
    close: (paneId, hostId) =>
      ipcRenderer.invoke("rdp-close", { paneId, hostId }),
  ...
  bmc: {
    ...
    open: (paneId, hostId) =>
      ipcRenderer.invoke("bmc-open", { paneId, hostId }),
    ...
    close: (paneId, hostId) =>
      ipcRenderer.invoke("bmc-close", { paneId, hostId }),
  ...
  webdavSync: {
    ...
    configure: (patch) =>
      ipcRenderer.invoke("webdav-sync-configure", patch || {}),
    test: (opts) => ipcRenderer.invoke("webdav-sync-test", opts || {}),
    setEnabled: (enabled) =>
      ipcRenderer.invoke("webdav-sync-set-enabled", enabled),
    ...
    reportSettings: (settings) =>
      ipcRenderer.invoke("webdav-sync-settings", settings),
    ...
    restoreBackup: (name) =>
      ipcRenderer.invoke("webdav-sync-restore-backup", { name }),
    deleteBackup: (name) =>
      ipcRenderer.invoke("webdav-sync-delete-backup", { name }),
    resetLocal: () => ipcRenderer.invoke("webdav-sync-reset-local"),
  },
  ...
  remoteEdit: {
    open: (tabId, remotePath) =>
      ipcRenderer.invoke("sftp-edit-open", { tabId, remotePath }),
    stop: (tabId, remotePath) =>
      ipcRenderer.invoke("sftp-edit-stop", { tabId, remotePath }),
  ...
  ai: {
    ...
    models: ({ refresh = false } = {}) =>
      ipcRenderer.invoke("ai-model-list", { refresh }),
    ...
    start: ({ scope, sessionId, sessionIds, hostIds } = {}) =>
      ipcRenderer.invoke("ai-conversation-start", {
        scope,
        sessionId,
        sessionIds,
        hostIds,
    ...
    history: (conversationId) =>
      ipcRenderer.invoke("ai-conversation-history", conversationId),
    // Releases the running query and keeps the transcript, so the
    // conversation can be picked up again from the history menu.
    park: (conversationId) =>
      ipcRenderer.invoke("ai-conversation-park", conversationId),
    close: (conversationId) =>
      ipcRenderer.invoke("ai-conversation-close", conversationId),
    ...
    send: (conversationId, text) =>
      ipcRenderer.invoke("ai-send", { conversationId, text }),
    interrupt: (conversationId) =>
      ipcRenderer.invoke("ai-interrupt", conversationId),
    ...
    approve: (requestId, approved, message) =>
      ipcRenderer.invoke("ai-approval-response", {
        requestId,
        approved,
        message,
      }),
